...
# Extract relevant columns and compare
...
Excellent! We have safely merged our metadata and alpha diversity data into a single data frame and we are more confident now that the data from the correct samples were merged together. Let's plot the relationship between a patient's Shannon index and their BMI.

```{r}
plot(meta_alpha$BMI, meta_alpha$shannon, xlab="Body Mass Index", ylab="Shannon Diversity Index", xlim=c(0,60))
```

#### Activity 3
Based on what we learned in the earlier exercises let's make some improvements.
* Alter the limits of the x-axis to go from 0 to 60 and have the y-axis go from 0 to 5.
* Color the points by diagnosis

```{r}
colors <- c(normal="black", adenoma="blue", carcinoma="red")
plot(meta_alpha$BMI, meta_alpha$shannon, xlim=c(0,60), ylim=c(0,5), xlab="Body Mass Index", ylab="Shannon Diversity Index", col=colors[meta_alpha$dx], pch=19)
```

Let's take a brief aside to re-factor our plotting code. Take a look at what we have here

```{r}
plot(meta_alpha$shannon~meta_alpha$BMI, xlim=c(0,60), ylim=c(0,5), xlab="Body Mass Index", ylab="Shannon Diversity Index", col=colors[meta_alpha$dx], pch=19)
```

There's a subtle change here, do you see it? The output is the same, but in this code we used a `~`. Numerous commands in R will allow us to use the `~`. This should be read as "shannon is explained by BMI" or "our y-axis variable is explained by our x-axis variable". The significance isn't so obvious in this example. By doing this, however, we can use the `data` argument to simplify our code.

```{r}
plot(shannon~BMI, data=meta_alpha, xlim=c(0,60), ylim=c(0,5), xlab="Body Mass Index", ylab="Shannon Diversity Index", col=colors[dx], pch=19)
```
...
symbol <- c(17, 19)
plot(sobs~fit_result, data=meta_alpha, xlim=c(0,3000), ylim=c(0, 400), xlab="Fit Result", ylab="Number of OTUs", col=colors[Gender], pch=symbol[Smoke+1])
legend("bottomright", legend=c('Female smoker','Male smoker', 'Female non-smoker', 'Male non-smoker'))
```
...
symbol <- c(17, 19)
plot(sobs~fit_result, data=meta_alpha, xlim=c(0,3000), ylim=c(0, 400), xlab="Fit Result", ylab="Number of OTUs", col=colors[Gender], pch=symbol[Smoke+1])
legend(x=1000, y=100, legend=c('Female smoker','Male smoker', 'Female non-smoker', 'Male non-smoker'))
```
