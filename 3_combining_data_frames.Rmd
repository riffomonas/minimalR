...
Excellent! We have safely merged our metadata and alpha diversity data into a single data frame and we are more confident now that the data from the correct samples were merged together. Let's plot the relationship between a patient's Shannon index and their age.

```{r}
plot(meta_alpha$Age, meta_alpha$shannon, xlab="Age (years)", ylab="Shannon Diversity Index", xlim=c(0,60))
```
...
plot(meta_alpha$Age, meta_alpha$shannon, xlim=c(0,60), ylim=c(0,5), xlab="Age (years)", ylab="Shannon Diversity Index", col=colors[meta_alpha$dx], pch=19)
```

Let's take a brief aside to re-factor our plotting code. Take a look at what we have here

```{r}
plot(meta_alpha$shannon~meta_alpha$Age, xlim=c(0,60), ylim=c(0,5), xlab="Age (years)", ylab="Shannon Diversity Index", col=colors[meta_alpha$dx], pch=19)
```
...
```{r}
plot(shannon~Age, data=meta_alpha, xlim=c(0,60), ylim=c(0,5), xlab="Age (years)", ylab="Shannon Diversity Index", col=colors[dx], pch=19)
```
