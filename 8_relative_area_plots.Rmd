...
To this point our analysis has looked at alpha and beta diversity values for our samples. We'd like to start drilling into the data to get a better sense of the types of organisms that might differ between the three diagnosis groups. Some of the general methods presented in this session (e.g. string manipulation) are widely used analyzing microbiome data. It's also important to note that the graphics approaches are actually pretty dreadful. They are presented for didactic purposes and to motivate other methods of visualizing taxonomic data.

What we'd like to do is to generate some pie charts and stacked bar charts to describe the biodiversity of the three diagnosis groups. One significant problem with pie and stacked bar charts is that the number of proportions you can represent *should* be rather limited. If we want to represent OTU data in a pie chart we would need hundreds of wedges. That would be a pain. To give a broad overview of the biodiversity, let's use the phylum-level abundances to build our plots. We'll need to manipulate the data in three steps. First, in the `data` folder there is a file called `baxter.cons.taxonomy` that contains the taxonomic information for each OTU that was observed when defining OTUs at a 3% distance cutoff. To build our plots we need to find the different phyla in that file. Second, we need to aggregate the data in the `baxter.shared` file by those phyla. Third, we will probably need to aggregate the data for each subject across individuals within a diagnosis group. Ready? Let's go!
...
```{r}
phylum <- gsub("Bacteria;([^;]*);.*", "\\1", tax)
otu_phylum <- data.frame(otu = taxonomy$OTU, phylum = phylum)
```
...
We've achieved step 1 - find the different phyla in the consensus taxonomy file. Next, we need to aggregate the columns in `baxter.shared` that come from the same phylum. Let's look at two ways of doing this and you can decide which you think is better.
...
```{r}
shared <- read.table(file="data/baxter.shared", header=T, stringsAsFactors=F, row.names=2)
shared <- shared[,-c(1,2)]
```

To get the names of the unique phylum names in `otu_phylum$phylum` we can use the `levels` function that we used previously (see Session 4).

```{r}
unique_phyla <- levels(otu_phylum$phylum)
```
...
```{r}
otus <- otu_phylum[otu_phylum$phylum == "Firmicutes", 'otu']
sub_shared <- shared[,otus]
```

Now we have a new data frame with 490 rows and 6585 columns. To sum across a data frame like this we can use the `apply` function. See if you can figure out what the apply function is doing in these two cases. Which is what we want?
...
If we run `count_phyla("Firmicutes", shared, otu_phylum)` we should get a vector of counts indicating the number of sequences in each sample that affiliated with the Firmicutes. We now need to loop this function over all of the phyla. To do this, we first need to initialize a new data frame that will hold the data.
...
for(phylum in unique_phyla){
	phylum_shared[,phylum] <- count_phyla(phylum, shared, otu_phylum)
}
```
...
As we saw in the first session, `dim` returns the number of rows and columns in a data frame. We aren't quite sure what "positive length" means, but there seems to be a problem somewhere in our function. If we enter `phylum` at the terminal we'll get back "Chloroflexi". This tells us that the last value of `phylum` that was tried in our for loop was for the Chloroflexi. Now what we can do is to recreate the manual calculation like we did above for the "Firmicutes" and see if we find an error.

```{r}
otus <- otu_phylum[otu_phylum$phylum == "Chloroflexi", 'otu']
sub_shared <- shared[,otus]
phylum_count <- apply(sub_shared, 1, sum)
```
...
Go ahead and look at the `rel_abund` matrix. To make it easier to look at, we'll order the columns by the average relative abundance across the subjects. Again, we'll start with the apply function. Then we'll use the `order` function to find the proper ordering. Finally, we'll reorder the matrix.

```{r}
ave_rel_abund <- apply(rel_abund, 2, mean)
decreasing_order <- order(ave_rel_abund, decreasing=T)
...
```{r}
ave_phylum <- aggregate(rel_abund, by=list(metadata$dx), mean)
```

The key difference is that we tell `aggregate` explicitly what to aggregate by and this needs to be a `list` of values. The output is a data frame where the diagnoses are the rows and the phyla are the columns. We'd like to convert this to a matrix where the rows are the phyla are the rows and the diagnoses are the columns. We'll create `ave_phylum_matrix`, which is a matrix containing the transpose (`t`) of the `ave_phylum` data frame with out the `Group.1` column

```{r}
ave_phylum_matrix <- as.matrix(t(ave_phylum[,-1]))
colnames(ave_phylum_matrix) <- ave_phylum$Group.1
```

Now we can make a pie chart for our data.

```{r}
pie(ave_phylum_matrix[,"normal"], col=rainbow(n=nrow(ave_phylum_matrix)))
pie(ave_phylum_matrix[,"normal"], col=rainbow(n=nrow(ave_phylum_matrix)))
pie(ave_phylum_matrix[,"normal"], col=rainbow(n=nrow(ave_phylum_matrix)))
```

Let's see an alternative way to represent relative abundance data using relative areas - stacked barplots. We've previously seen how to generate bar plots when we were plotting alpha diversity data. When we used `barplot` previously, we used `beside=T` to plot the bars next to each other. If we instead use `beside=F`, which is the default, we get a stacked bar plot.

```{r}
barplot(ave_phylum_matrix, col=rainbow(n=nrow(ave_phylum_matrix)), names.arg=c("Normal", "Adenoma", "Cancer"), legend=T)
```

I don't want to spend a lot of time on the various parameters that are available for pie and stacked bar plots. They're a data visualization eyesore. If you run `?pie` You'll find the following gem in the Note section...
